...
'---------------------------------------------------------
 Public Sub prtTI_Monitor(Msg As String)
'---------------------------------------------------------
...
Select Case prtEtat
    ...
    Case "SA": x = "Crédits documentaires soldés (TI = S36) "
    Case "SI": x = "Crédits documentaires non soldés (TI = S36)"
    Case "UP": x = "Crédoc :( Util - Paiement < 5 %) "
    Case Else: x = "Crédits documentaires "
End Select

prtTitleText = x & " : Validité au " & dateImp(paramTIDB2_AMJValidité)
    ...
    If intReturn = 0 Then
        ...
        Else
            If Not blnDevise Or (blnDevise And selCDComD_Devise = recCDDossier.Devise) Then
                ...
                If Trim(recCDDossier.AMJSituation) = "" And recCDDossier.AMJValidité <= paramTIDB2_AMJValidité Then
                    curSolde = recCDDossier.MontantEngagement - recCDDossier.MontantUtilisé
                     ...
                     Select Case prtEtat
                        ...
                        Case "SA": If curSolde = 0 And curS36Solde = curSolde Then prtTI_Line
                        
                        Case "SI": If curSolde <> 0 And curS36Solde = curSolde Then prtTI_Line
                        
                        Case "SD":
                             'If recCDDossier.s36Engagement <> recCDDossier.MontantEngagement _
                             'Or recCDDossier.curS36Solde <> curSolde Then
                                 If curS36Solde <> curSolde Then prtTI_Line
                        Case "UP":
