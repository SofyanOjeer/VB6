...
Public Function isEligbleFGDR(madb As ADODB.Connection, lClient As String, lCompte As String) As Boolean
Dim xSQL As String
Dim rsDenis As ADODB.Recordset
Dim vo_info  As ADODB.Recordset

    isEligbleFGDR = False
    madb.Execute "USE VUC_V4"
    Set vo_info = New ADODB.Recordset
    xSQL = "SELECT * FROM VUC_Clients where client=" & CLng(lClient)
    xSQL = xSQL & " AND compte='" & lCompte & "'"
    Call vo_info.Open(xSQL, madb, adOpenStatic)
    If Not vo_info.EOF Then
        xSQL = "select '??' from " & paramIBM_Library_SAB & ".ZCLIENA0, " & paramIBM_Library_SAB & ".ZCOMPTE0 where CLIENACLI = '" & lClient & " '"
        xSQL = xSQL & " AND CLIENACAT IN('PAR','PRR','PER','GAR','PAV','BQG','BQE','STE','EI','ASR','ASS','AMB','ADM')"
        xSQL = xSQL & " AND CLIENAETA IN('MMME','M/ME','MRS','MR','MME','MLLE','EIMR','BANQ','SARL','Sa','SA','SETR','SAS','SNC','SCV','AMBA','SEP','SCI','ASSO','SCS','EURL','SCA','PMAD','SA D')"
        xSQL = xSQL & " AND COMPTECOM = '" & lCompte & "'"
        xSQL = xSQL & " AND (LEFT(COMPTEOBL, 3) = '255' OR LEFT(COMPTEOBL, 4) = '2511' OR LEFT(COMPTEOBL, 3) = '262' OR LEFT(COMPTEOBL, 3) = '253')"
        'xSQL = xSQL & " AND COMPTEDEV in ('EUR','GBP','BGN','HRK','DKK','HUF','ISK','LVL','LTL','NOK','PLN','CZK','RON','SEK')"
        Set rsDenis = cnsab.Execute(xSQL)
        If Not rsDenis.EOF Then
            If rsDenis(0) = "??" Then isEligbleFGDR = True
        End If
        ...
        Set rsDenis = Nothing
    End If
    If vo_info.State = adStateOpen Then
        vo_info.Close
    End If
    Set vo_info = Nothing
...
Public Sub prtYBIAMVT0_A4_OpenX_ResetPDF()
...
Public Sub prtYBIAMVT0_A4_InfoFGDR()
    ...
    XPrt.FontSize = 7
    XPrt.CurrentY = Line4 + 1175
    XPrt.ForeColor = RGB(0, 0, 160)
    XPrt.CurrentX = col1 + 50
    XPrt.Print "Votre compte est éligible à la garantie du Fonds de Garantie des Dépôts et de Résolution selon les textes en vigueur.";
...
Private Sub prtYBIAMVT0_A4_Médiateur()
Dim X As String

    XPrt.FontSize = 7
    XPrt.CurrentY = Line4 + 1400
    XPrt.ForeColor = RGB(0, 0, 160)
    XPrt.FontBold = False
    XPrt.CurrentX = col1 + 50
...
End Sub
Public Function prtZBAGFAC0_A4_Extrait_FraisPDF(lMOUVEMCOM As String, lAMJMin As String, lAMJMax As String, blnCSV As Boolean, lstErr As ListBox, lRELEVEREL As String, lRéférence As String, blnNewPage As Boolean) As Boolean
Dim rsLocal As ADODB.Recordset, rsW As ADODB.Recordset
Dim xSQL As String
Dim Nb As Integer
Dim CTLAMJ As String
...
Dim wAMJ_Solde As String
    ...
    prtZBAGFAC0_A4_Extrait_FraisPDF = False
    valAmjMin = lAMJMin
    valAmjMax = lAMJMax
    IbmAmjMin = dateIBM(lAMJMin)
    IbmAmjMax = dateIBM(lAMJMax)
    ...
    If Not rsLocal.EOF Then
        ...
        Do Until rsLocal.EOF
                If xZBAGFAC0.BAGFACDOP >= IbmAmjMin Then
                    If Not blnA4_Form Then
                        prtZBAGFAC0_A4_OpenX_ResetPDF
                        Call prtZBAGFAC0_A4_Form("Édité le " & dateImp(DSys), "")
                        blnNewPage = True
                        prtZBAGFAC0_A4_Extrait_FraisPDF = True
                    End If
                    If nbLigne = NbLigneMax Then
                        prtZBAGFAC0_A4_Report
                        lstErr.RemoveItem lstErr.ListCount - 1
                    ...
                    nbLigne = nbLigne + 1
                    prtZBAGFAC0_A4_Line
                    DoEvents: If prtKillDoc Then frmElpPrt.Hide: Exit Function
                End If
           rsLocal.MoveNext
           Call rsZBAGFAC0_GetBuffer(rsLocal, xZBAGFAC0)
        Loop
...
Public Sub prtZBAGFAC0_A4_OpenX_ResetPDF()
'Dim xXPrt As Printer
'
'    imprimanteParDefaut = Printer.Devicename
'    If nomDuServeur <> paramServerSplf Then
'        For Each xXPrt In Printers
'            X = UCase$(Trim(xXPrt.Devicename))
...
Call MsgBox(Err & " : " & Error(Err), vbCritical, "prtYBIAMVT0_A4_Close")
frmElpPrt.Hide
...
'---------------------------------------------------------
Public Sub prtYBIAMVT0_A4_Form(Msg As String, lRéférence As String, dateDU As String, dateAU As String)
'---------------------------------------------------------
Dim X As String
Dim mCurrenty
...
XPrt.FontSize = 8
XPrt.CurrentY = Line1 - prtlineHeight * 4 + 80
XPrt.FontBold = False
XPrt.CurrentX = col3 + 200
XPrt.Print "Période du ";
XPrt.FontBold = True
XPrt.Print dateDU;
XPrt.FontBold = False
XPrt.Print " au ";
...
'---------------------------------------------------------
Public Sub prtYBIAMVT0_A4_Line(dateDU As String, dateAU As String)
'---------------------------------------------------------
Dim X As String, I As Integer, libCV As String, blnCV As Boolean
Dim blnLine2 As Boolean, xLine1 As String, xLine2 As String
Dim kJust As Integer, kMax As Integer
...
Else
    ...
    XPrt.Print xLine1; ' & " -";
    If nbLigne = NbLigneMax Then prtYBIAMVT0_A4_Report dateDU, dateAU
    nbLigne = nbLigne + 1
    XPrt.CurrentY = XPrt.CurrentY + prtlineHeight
    XPrt.CurrentX = col2 + 50
    XPrt.Print xLine2;
...
'---------------------------------------------------------
Public Sub prtYBIAMVT0_A4_Médiateur_OLD()
'---------------------------------------------------------
Dim X As String
XPrt.FontSize = 7
...
'---------------------------------------------------------
Public Sub prtYBIAMVT0_A4_Pauget_Constans(lRELEVEREL As String, dateDU As String, dateAU As String)
'---------------------------------------------------------
Dim X As String
Dim dateTaux As String
...
If blnPauget_Constans Then 'blnMédiateur Then
    ...
    XPrt.Print dateAU;
    XPrt.FontBold = False
    XPrt.Print ", le Total des frais sur vos opérations courantes relevant de la convention de compte est de ";
    XPrt.FontBold = True
...
If mZAUTSYC0.AUTSYCMON <> 0 Then
    ...
    dateTaux = prtYBIAMVT0_A4_Pauget_Constans_TAEG
    XPrt.FontBold = False
    If blnMédiateur Then
        XPrt.Print " au Taux Effectif Annuel Global (TAEG) de ";
    Else
        XPrt.Print ". Au ";
        XPrt.FontBold = True
        XPrt.Print dateTaux;
        XPrt.FontBold = False
        XPrt.Print ", le Taux Effectif Global (TEG) applicable s'élève à ";
    End If
...
If lRELEVEREL = "M" Then
    mExtraitNuméro = libMois(Mid$(lAMJMax, 5, 2)) & " " & Left(lAMJMax, 4)
    lRéférence = ""
Else
           ...
           If xYBIAMVT0.MOUVEMDTR > IbmAmjMax Then Exit Do
               ...
               End If
                ...
                If xYBIAMVT0.MOUVEMDTR >= IbmAmjMin Then
                    ...
                    If Not blnA4_Form Then
                        prtYBIAMVT0_A4_Form "Solde au : " & dateImp(wAMJ_Solde), lRéférence, dateAAAAMMJJTOJJ_MM_AAAA(lAMJMin), dateAAAAMMJJTOJJ_MM_AAAA(lAMJMax)
                        blnNewPage = True
                    End If
                    If nbLigne = NbLigneMax Then
                        prtYBIAMVT0_A4_Report dateAAAAMMJJTOJJ_MM_AAAA(lAMJMin), dateAAAAMMJJTOJJ_MM_AAAA(lAMJMax)
                        lstErr.RemoveItem lstErr.ListCount - 1
                        lstErr.AddItem xYBIAMVT0.MOUVEMCOM & " page : " & NbPage
                    End If
                    nbLigne = nbLigne + 1
                    
                    prtYBIAMVT0_A4_Line dateAAAAMMJJTOJJ_MM_AAAA(lAMJMin), dateAAAAMMJJTOJJ_MM_AAAA(lAMJMax)
...
If Not blnA4_Form Then
    ...
    prtYBIAMVT0_A4_Form "Solde au : " & dateImp(dateElp("Jour", -1, lAMJMin)), lRéférence, dateAAAAMMJJTOJJ_MM_AAAA(lAMJMin), dateAAAAMMJJTOJJ_MM_AAAA(lAMJMax)
...
If blnA4_Form Then
    ...
    If blnPauget_Constans Then
        prtYBIAMVT0_A4_Pauget_Constans lRELEVEREL, dateAAAAMMJJTOJJ_MM_AAAA(lAMJMin), dateAAAAMMJJTOJJ_MM_AAAA(lAMJMax)
    Else
        If blnFRS_Info Then
            XPrt.ForeColor = RGB(0, 0, 160)
            XPrt.CurrentX = col2
    ...
    If Trim(meYBIACPT0.CLIENACLI) <> "" Then
        If isEligbleFGDR(madb, meYBIACPT0.CLIENACLI, meYBIACPT0.COMPTECOM) Then
...
Public Sub prtYBIAMVT0_A4_Report(dateDU As String, dateAU As String)
XPrt.CurrentY = Line4 + 50
prtYBIAMVT0_A4_Montant (solde)
NbPage = NbPage + 1
frmElpPrt.prtNewPage
prtYBIAMVT0_A4_Form "Report", "", dateDU, dateAU
...
Public Function prtYBIAMVT0_A4_Pauget_Constans_TAEG() As String
Dim rsW As ADODB.Recordset, xSQL As String
...
'======================================================================
prtYBIAMVT0_A4_Pauget_Constans_TAEG = DateAdd("d", -1, Now)
s = Split(prtYBIAMVT0_A4_Pauget_Constans_TAEG, " ")
prtYBIAMVT0_A4_Pauget_Constans_TAEG = s(0)
...
If blnOk Then
    ...
    newdate = arrECHTAB(arrECHTAB_K).AMJ7
    prtYBIAMVT0_A4_Pauget_Constans_TAEG = dateAAAAMMJJTOJJ_MM_AAAA(newdate)
End If
End Function
...
Function retourne_mediateur(numCli As String) As Boolean
Dim xSQL As String
Dim rsDenis As ADODB.Recordset

    retourne_mediateur = False
    xSQL = "SELECT CLIENACAT FROM " & paramIBM_Library_SAB & ".ZCLIENA0 WHERE CLIENACLI = '" & numCli & "' AND CLIENACAT IN('ASS','PAV','PAR','PER','PRR','TIE')"
    Set rsDenis = cnsab.Execute(xSQL)
    If Not rsDenis.EOF Then
        retourne_mediateur = True
    End If
    If rsDenis.State = adStateOpen Then
        rsDenis.Close
    End If
