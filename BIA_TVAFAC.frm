      ...
      Begin VB.Frame fraNIF 
         ...
         Begin VB.Frame fraNIF_Options_1 
            ...
            Begin VB.Label libUpdate_TVANIFCLIT_URL 
               Alignment       =   2  'Center
               Caption         =   "http://ec.europa.eu/taxation_customs/vies/vieshome.do"
               BeginProperty Font 
            ...
            Begin VB.Label libUpdate_TVANIFCLIT 
               Alignment       =   2  'Center
               BackStyle       =   0  'Transparent
               Caption         =   "Le site suivant permet de vérifier la validité du n° de TVA , cliquez sur le lien pour y accéder "
               BeginProperty Font 
                  Name            =   "@Arial Unicode MS"
                  Size            =   10.5
                  Charset         =   0
...
Dim mTVA_DES_File As String, mTVA_DES_File_Local As String
...
txtSelect_TVAFACDTR_Max.Visible = False
txtSelect_TVAFACSTA.Enabled = False
txtSelect_TVAFACCLIC.Enabled = True
txtSelect_TVAFACCLI.Enabled = True
chkSelect_TVAFACSTA.Visible = False
If cmdSelect_SQL_K = 8 Then
    Call DTPicker_Set(txtSelect_TVAFACDTR, YBIATAB0_DATE_CPT_MP1)
Else
    Call DTPicker_Set(txtSelect_TVAFACDTR, YBIATAB0_DATE_CPT_MP1)
    chkSelect_TVAFACDTR.Enabled = True
    txtSelect_TVAFACCLI.Enabled = False
    txtSelect_TVAFACCLIC.Enabled = False
End If
...
blnControl = False
usrColor_Set
libUpdate_TVANIFCLIT_URL.ForeColor = vbMagenta
...
txtUpdate_TVAFACCLIC.AddItem "E tiers remdoc"
...
txtSelect_TVAFACCLIC.AddItem "E tiers remdoc"
...
txtUpdate_TVACOMCLIC.AddItem "E tiers remdoc"
...
txtDétail_TVACOMCLIC.AddItem "E tiers remdoc"
...
txtUpdate_TVANIFCLIC.AddItem "E tiers remdoc"
...
txtNIF_TVANIFCLIC.AddItem "E tiers remdoc"
...
If InStr(1, X, "PDF") <= 0 Then
    Call MsgBox("Choisir l'imprimante 'PDF'", vbCritical, "Archivage des factures émises")
    Exit Sub
End If
...
If Not msFileSystem.FolderExists(paramFacturation_Path_AAAA) Then MkDir paramFacturation_Path_AAAA
...
Set rsSab = Nothing
Call lstErr_Clear(lstErr, cmdContext, "cmdSelect_SQL_8"): DoEvents
currentAction = "cmdSelect_SQL_8"
X = UCase$(Trim(Printer.Devicename))
If InStr(1, X, "PDF") > 0 Then
    Call MsgBox("Choisir une imprimante non 'PDF'", vbCritical, "Impression des factures ")
    Exit Sub
'    Call MsgBox("TEST désactivé", vbCritical, "Archivage des factures émises") '$JPL 20141203 pour reprise Archivage
End If
...
X = UCase$(Trim(Printer.Devicename))
If InStr(1, X, "PDF") = 0 Then
    X = MsgBox("Attention l'imprimante n'est pas'PDF', voulez-vous continuer ?", vbQuestion & vbYesNo, "Etat de déclaration de la TVA intracommunautaire aux douanes")
    If X <> vbYes Then Exit Sub
...
If wAMJMin < 20100000 Then
    X = MsgBox("Impossible, la date de facturation est < à 2010: " & dateImp(wAMJMin), vbCritical, "Etat de déclaration de la TVA intracommunautaire aux douanes")
    Exit Sub
...
xSQL = "select count(*) from " & paramIBM_Library_SABSPE & ".YTVAFAC0 " _
      & " where TVAFACETA > 0 and TVAFACSTA = 'F' and TVAFACCLIP <> 'FR' and TVAFACDTR = " & mTVAFACDTR
Set rsSab = cnsab.Execute(xSQL)
If Not rsSab.EOF Then Nb = rsSab(0)
xSQL = "select count(*) from " & paramIBM_Library_SABSPE & ".YTVAFAC0 " _
      & " where TVAFACETA > 0 and TVAFACSTA = 'F'  and TVAFACDTR = " & mTVAFACDTR
Set rsSab = cnsab.Execute(xSQL)
If Not rsSab.EOF Then Nb_CLIT = rsSab(0)

X = MsgBox("confirmez-vous la date de facturation : " & dateImp(wAMJMin) _
           & vbCrLf & vbCrLf & Nb & " / " & Nb_CLIT & " factures à déclarer", vbQuestion & vbYesNo, "Etat de déclaration de la TVA intracommunautaire aux douanes")
If X <> vbYes Then Exit Sub


mTVA_DES_File = paramFacturation_Path & "TVA_DES\TVA_DES_" & wAMJMin & " en date du " & DSys & "_" & time_Hms & ".xml"
mTVA_DES_File_Local = "C:\Temp\TVA DES " & Mid$(wAMJMin, 5, 2) & " " & Mid$(wAMJMin, 1, 4) & ".xml"
Call FEU_ROUGE
...
K = Val(Mid$(wAMJMin, 1, 4)) - 2010 + Val(Mid$(wAMJMin, 5, 2))
Print #1, "               <num_des>" & Format$(K, "00000") & "</num_des>"
...
If Trim(Dir(mTVA_DES_File_Local)) <> "" Then Kill mTVA_DES_File_Local
msFileSystem.CopyFile mTVA_DES_File, mTVA_DES_File_Local
Call FEU_VERT
...
newYTVAFAC0 = oldYTVAFAC0
If xYTVAFAC0.TVAFACSTA = "0" Or xYTVAFAC0.TVAFACSTA = "1" Or xYTVAFAC0.TVAFACSTA = "9" Then
    ...
    Else
        If Mid$(X, 1, 2) <> oldYTVAFAC0.TVAFACCLIP Then
            If MsgBox("Code pays résidence <> code pays NIF" & vbCrLf & " confirmez-vous le code NIF saisi" _
                , vbYesNo + vbQuestion, "Contrôle du code NIF") = vbNo Then
                blnUpdate_Control = False
                txtUpdate_TVAFACCLIT.BackColor = errUsr.BackColor
...
Private Sub libUpdate_TVANIFCLIT_URL_Click()
Call frmElpPrt.IExplore(libUpdate_TVANIFCLIT_URL)
...
For K = 1 To arrYTVACOM0_Nb
        ...
        If X = "=ECRX" Then
            ...
            If IsNull(V) Then
                If Len(Trim(xYTVACOM0.TVAREFCLI)) = 0 Then
                    X = Trim(meYBIAMVT0.LIBELLIB1)
                    XPrt.Print X;
                'DR 06/05/2014
                ElseIf (Trim(xYTVACOM0.TVACOMOPE) = "CDE" Or Trim(xYTVACOM0.TVACOMOPE) = "CDI" Or Trim(xYTVACOM0.TVACOMOPE) = "RDE" Or Trim(xYTVACOM0.TVACOMOPE) = "RDI") Then
                    X = "V/Ref " & Trim(xYTVACOM0.TVAREFCLI)
                    XPrt.Print X & "  ";
                    aFont = XPrt.FontSize
                    XPrt.FontSize = 6
                     X = Trim(meYBIAMVT0.LIBELLIB1)
                    'If Len(Trim(X)) > 24 Then X = Left(X, 24)
                    XPrt.Print X;
                   XPrt.FontSize = aFont
                Else
                    X = Trim(meYBIAMVT0.LIBELLIB1)
        ...
        Else
            For arrCommission_K = 1 To arrCommission_Max
                If X = arrCommission(arrCommission_K, 1) Then
                    'xxxxDenis 26/04/2010
                    If Len(Trim(xYTVACOM0.TVAREFCLI)) = 0 Then
                        X = Trim(arrCommission(arrCommission_K, 2))
                        XPrt.Print X;
                    Else
                        X = "V/Ref " & Trim(xYTVACOM0.TVAREFCLI)
                        XPrt.Print X & "  ";
        ...
        If mTVAFACCLIP_Code_Fiscal = "4" Or mTVAFACCLIP_Code_Fiscal = "5" Then
            ...
            End If
        End If
        If xYTVACOM0.TVACOMMTVE <> 0 And xYTVACOM0.TVACOMTVAC = "E" Then xYTVACOM0.TVACOMTVAC = "N"
...
If nbHT_N > 0 Then
    ...
    X = "Totaux HT et TVA des prestations soumises au taux de TVA normal en vigueur"
...
If nbHT_R > 0 Then
    ...
    XPrt.ForeColor = RGB(0, 123, 141)
    X = "Totaux HT et TVA des prestations soumises au taux de TVA réduit en vigueur"
    XPrt.CurrentX = prtMinX + 8100 - XPrt.TextWidth(X)
    XPrt.Print X;
    XPrt.ForeColor = prtForeColor
...
If nbHT_N > 0 Then
    XPrt.CurrentY = XPrt.CurrentY + prtlineHeight
    XPrt.CurrentX = prtMinX + 50
    XPrt.Print "N : TVA à " & tauxTVA_Lib & " (taux normal)";
End If
If nbHT_R > 0 Then
    XPrt.CurrentY = XPrt.CurrentY + prtlineHeight
    XPrt.CurrentX = prtMinX + 50
    XPrt.Print "R : TVA (taux réduit)";
End If
If nbHT_E > 0 Then
    XPrt.CurrentY = XPrt.CurrentY + prtlineHeight
    XPrt.CurrentX = prtMinX + 50
...
If cmdSelect_SQL_K = 7 Then
    ...
    '----------------------------------------------
Else
   '$JPL 20141203 pour reprise Archivage prtPgmName = paramFacturation_Path_AAAA & prtPgmName '$JPL 20141203 pour reprise Archivage
    prtBIA_TVAFAC_Close 1
End If
...
wSendMail.Message = "<body bgcolor = #FFFFFF>" _
                    ...
                    & "<BR><BR><Font color = #000080> Le fichier .xml est archivé  : <Font color = #FF0000>" & Replace(mTVA_DES_File, "\\", "\") _
                    & "<BR><Font color = #000080>Une copie du fichier .xml est enregistré localement: <Font color = #FF00FF>" & "\" & usrIdNT & "\" & mTVA_DES_File_Local _
...
Select Case Mid$(txtNIF_TVANIFCLIC, 1, 1)
    ...
    Case "D", "R", "E": wRecipient = "SOBI"
    Case "G": wRecipient = "GDMP"
    Case Else: wRecipient = "INFO"
End Select

X = Table_Unit_SSI("", Trim(wRecipient))
Call frmElpPrt.prtIMP_PDF_NoPaper_Init(X, "BIA-TVA-NIF-" & wRecipient, "Prod")
...
If blnAuto Then Call frmElpPrt.prtIMP_PDF_NoPaper_Mail("", "", "Facturation TVA : Liste des numéros de TVA intracommunautaire - " & wRecipient)
...
If blnAuto Then
    ...
    Call frmElpPrt.prtIMP_PDF_NoPaper_Mail("", "", " Facturation TVA : Liste des commissions à réviser - " & meYTVACOM0.TVACOMSRVR)
...
For K = 1 To arrYTVACOM0_Nb
    ...
    If xYTVACOM0.TVACOMSRVR <> meYTVACOM0.TVACOMSRVR Then
        ...
        meYTVACOM0.TVACOMCLIC = "?"
        X = Table_Unit_SSI("", Trim(xYTVACOM0.TVACOMSRVR))
        Call frmElpPrt.prtIMP_PDF_NoPaper_Init(X, "BIA-TVA-COM-" & meYTVACOM0.TVACOMSRVR, "Prod")
...
If xYTVAFAC0.TVAFACSTA = "0" Or xYTVAFAC0.TVAFACSTA = "9" Then
    txtUpdate_TVAFACCLIT.Enabled = True
Else
    txtUpdate_TVAFACCLIT.Enabled = False
End If
If Trim(xYTVAFAC0.TVAFACCLIT) = "" Then
    If xYTVAFAC0.TVAFACSTA = "0" Or xYTVAFAC0.TVAFACSTA = "9" Then
...
txtUpdate_TVAFACCLIT = Trim(xYTVAFAC0.TVAFACCLIT)
...
If X = "ORPA" Then X = "GDMP"  '$JPL 2014-10-13 (pb P. SALLE = 2 services)
...
If newYTVAFAC0.TVAFACCLIC = " " Then
    xSQL = "select CLIFISNIF from " & paramIBM_Library_SAB & ".ZCLIFIS0 " _
         ...
         & " and   CLIFISCLI = '" & newYTVAFAC0.TVAFACCLI & "'" _
         & " and   CLIFISTYP = '1'"
...
If Trim(newYTVAFAC0.TVAFACCLIT) <> "" Then
    ...
    If Not IsNull(V) Then
        If cmdSelect_SQL_K = "1" Then
            Call MsgBox("La structure du NIF est erronnée : " & newYTVAFAC0.TVAFACCLIT, vbInformation, "Contrôle du NIF : " & newYTVAFAC0.TVAFACCLI)
        End If
...
Else
    If cmdSelect_SQL_K = "1" Then
        Call MsgBox("NIF manquant : " & newYTVAFAC0.TVAFACCLI, vbInformation, "Contrôle du NIF : " & newYTVAFAC0.TVAFACCLI)
    End If
End If
...
Select Case X
    ...
    Case "R": Call cmdNIF_SQL_1_ZENCTIE0("R")
    Case "E": Call cmdNIF_SQL_1_ZENCTIE0("E")
End Select
fgNIF_Display
...
Select Case Mid$(txtNIF_TVANIFCLIC, 1, 1)
    ...
    Case "R": Call cmdNIF_SQL_2_ZENCTIE0("R")
    Case "E": Call cmdNIF_SQL_2_ZENCTIE0("E")
...
Select Case Mid$(txtNIF_TVANIFCLIC, 1, 1)
    ...
    Case "R": Call cmdNIF_SQL_3_ZENCTIE0("R")
    Case "E": Call cmdNIF_SQL_3_ZENCTIE0("E")
...
Public Sub cmdNIF_SQL_2_ZENCTIE0(RE As String)
Dim V
Dim X As String, xSQL As String
On Error GoTo Error_Handler
...
Public Sub cmdNIF_SQL_3_ZENCTIE0(RE As String)
Dim V, blnTVANIFCLIT As Boolean, blnOk As Boolean
Dim X As String, xSQL As String
On Error GoTo Error_Handler
...
xSQL = "select distinct (TVACOMCLI),TVACOMCLIP,ENCTIERA1,ENCTIERA2,TVANIFCLIT,TVACOMCLIC from " _
    ...
    & " left join " & paramIBM_Library_SABSPE & ".YTVANIF0 on TVANIFCLI = TVACOMCLI and TVANIFCLIC = '" & RE & "'" _
    & " where TVACOMSTA not in ('F','I','X','A') and TVACOMCLIC = '" & RE & "'"
...
Do While Not rsSab.EOF
    ...
    xYTVANIF0.TVANIFRS = Trim(rsSab("CDOTIERA1")) & Trim(rsSab("CDOTIERA2"))
    xYTVANIF0.TVANIFCLIT = Trim(rsSab("TVANIFCLIT"))
    xYTVANIF0.TVANIFUPDS = Trim(rsSab("TVANIFUPDS"))
...
Public Sub cmdNIF_SQL_1_ZENCTIE0(RE As String)
Dim V
Dim X As String, xSQL As String
On Error GoTo Error_Handler
...
X = Trim(txtNIF_TVANIFCLI)
If IsNumeric(X) Then
    X = " where  TVANIFCLIC = '" & RE & "' and ENCTIETIE = '" & Format(X, "0000000") & "'"
Else
    X = " where TVANIFCLIC = '" & RE & "' and ENCTIERA1 like '%" & X & "%'"
End If
...
txtUpdate_TVANIFCLIT = Trim(xYTVANIF0.TVANIFCLIT)
If txtUpdate_TVANIFCLIT = "" Then
    If xYTVANIF0.TVANIFCLIP = "GR" Then
        txtUpdate_TVANIFCLIT = "EL"
    Else
        txtUpdate_TVANIFCLIT = xYTVANIF0.TVANIFCLIP
    End If
End If
...
V = TVANIFCLIT_Control(xYTVANIF0.TVANIFCLIT)
If Not IsNull(V) Then Call MsgBox(V, vbExclamation, "Contrôle de validité NIF")
Call lstErr_Clear(lstErr, cmdContext, ">Affichage du détail NIF"): DoEvents
End Sub
...
For K = 0 To arrPays_NB
    If lPays = arrPays(K).Id Then
        ...
        'xxxxDenis 14/10/2010
        'If arrPays(K).Fiscal <= "5" Then
        If arrPays(K).Fiscal <= "5" And arrPays(K).Fiscal <> "2" Then
